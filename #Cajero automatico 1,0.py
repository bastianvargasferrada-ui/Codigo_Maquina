...
if intento == pin:
      ...
      while opcion !=0:
        ...
        match opcion:
            ...
            case 4:
                  print("Cambio de pin")
                  intento =int(input("Ingrese su pin actual: "))
                  while intento != pin:
                        intento = int(input("Pin incorrecto, pruebe de nuevo: "))
                        contador_intentos_opc3 += 1
                        if contador_intentos_opc3 == 3:
                              print("Tarjeta bloqueada")
                              exit()
                        
                  else:
                        Nuevo_pin = int(input("ingrese su nuevo pin:"))
                        confirmacion_Pin = int(input("Ingrese su nuevo pin: "))
                        if confirmacion_Pin == Nuevo_pin:
                              pin = Nuevo_pin
                              print("¡Felicidades!, cambio de pin realizado exitosamente")
                              print("Su nuevo pin es", pin)
                        else:
                              print("Error al confirmar su pin, volviendo al menú")
                              print("...")
                              Nuevo_pin = pin
